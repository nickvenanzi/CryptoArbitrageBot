...
eth_price_usd = prices["0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"]
print(f"Price of ETH: ${eth_price_usd:2f}")
# Excel file setup
EXCEL_FILE = "./ethereum_transactions.xlsx"

COLUMNS = ["Block Number", "Tx Hash", "From", "To", "Gas Used", "Gas Price (Gwei)", "Gas (USD)", "Function"] + [f"Param {i+1}" for i in range(15)]
...
def monitor_blocks():
    """Monitor verified blocks and log transactions."""
    latest_block = w3.eth.block_number
    print(f"Monitoring transactions... (Starting from block {latest_block})")
    ...
    while True:
        ...
        if current_block > latest_block:
            ...
            txs = w3.eth.get_block(current_block, full_transactions=True).transactions
